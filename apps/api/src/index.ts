...
import { promises as fsPromises, stat as fsStat } from 'fs';
import path from 'path';
import { randomUUID } from 'crypto';

// A simple Fastify server implementing the backend API for Portal Lab.
//
// Authentication is mocked: upon login a "token" is issued which encodes
// the user's email, role and expiry in base64. The verifyToken function
// decodes this payload without any signature. In a real system you would
// replace this with proper JWT signing and verification.
...
function resolveRole(email: string): UserPayload['role'] {
  if (email === 'admin@demo.com') return 'admin';
  if (email === 'manager@demo.com') return 'manager';
  return 'viewer';
}
...
async function writeMetadata(items: any[]) {
...
function authGuard(allowedRoles?: UserPayload['role'][]): any {
...
app.put('/upload/:uploadId', { preHandler: authGuard(['manager', 'admin']) }, async (request, reply) => {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.raw.on('error', (err: Error) => reject(err));
  });
